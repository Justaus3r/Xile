...
while(True):    
    try:
        ...
        elif command.split()[0] == "getip":
            try:
                _argument_ = command.split()[1]
            except Exception:
                print("Local Ip address:",socket.gethostbyname(socket.gethostname()))   
                continue 
            if command.split()[0] == "getip" and int(len(command)) == 5:
                print("Local Ip address:",socket.gethostbyname(socket.gethostname()))
            elif command.split()[0] == "getip" and int(len(command)) > 5 and _argument_ == "-p":
                try:
